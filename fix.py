...
# 1. Insert the 3 economy objects at the start of the docs array
new_objects = """
  {
    title: "Tokenomics",
    slug: "tokenomics",
...
# Check if they are already in the file (just in case)
if 'slug: "staking-utility"' not in content:
    insert_str = 'export const docs: DocPage[] = ['
    insert_idx = content.find(insert_str)
    if insert_idx != -1:
        insert_idx += len(insert_str)
        content = content[:insert_idx] + "\n" + new_objects + content[insert_idx:]

# 2. Remove the FAQ section from Purr Points
faq_text_to_remove = "<h2>Frequently Asked Questions</h2><h3>When will I receive my $POKER tokens?</h3><p>The Token Generation Event (TGE) is planned for 2026. Your Purr Points will be converted to $POKER tokens at this time, proportional to your share of total points earned during Season 1.</p><h3>Can I see my current point balance?</h3><p>Yes! Your Purr Points are tracked in real-time and displayed on your profile within the Purr Poker app. You can also see your ranking relative to other players.</p><h3>Will there be more seasons after Season 1?</h3><p>Yes! The 65% community allocation is distributed across multiple seasons. Future seasons and their point structures will be announced after the TGE, continuing to reward active players.</p><h3>What happens if I'm flagged for Sybil activity?</h3><p>Accounts flagged for suspicious activity or attempting to game the system may have their points reduced or removed entirely. Play fair and you have nothing to worry about.</p>"

content = content.replace(faq_text_to_remove, "")
